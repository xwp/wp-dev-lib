...
function can_generate_coverage_clover {
	if [ -e .coveralls.yml ] && [ -e composer.json ] && check_should_execute 'coverage'; then
		if min_php_version "5.5.0" && cat composer.json | grep -Eq '"satooshi/php-coveralls"\s*:\s*"([(\^|~)]*(2|1.1)[0-9.]*|dev-master)"'; then
			return 0
		elif min_php_version "5.3.3" && cat composer.json | grep -Eq '"satooshi/php-coveralls"\s*:\s*"[(\^|~)]*[0-9.]*"'; then
			return 0
		fi
	fi
	return 1
